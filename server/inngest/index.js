...
const taskAssignmentEmail=inngest.createFunction(
            ...
            await step.run('check-if-task-is-completed',async()=>{
                ...
                if(task.status!=="DONE")
                {
                    await step.run('send-task-remainder-email',async()=>{
                        await sendEmail({
                            to:task.assignee.email,
                            subject:`Remainder for task ${ task.project.name }`,
                            body: `<div style="max-width: 600px;">
<h2>Hi ${task.assignee.name},👋</h2>

<p style="font-size: 16px;">You've task due in :< /p>
<p style="font-size: 18px; font-weight: bold; color: #007bff;
margin: 8px 0;">${task.title}</p>
...
export const functions = [syncUserCreation, syncUserDeletion, syncUserUpdation, syncWorkSpaceCreation, syncWorkSpaceUpdation, syncWorkSpaceDeletion, syncWorkSpaceMemberCreation, taskAssignmentEmail];
